...
class App(tk.Tk):
    ...
    def _build_help_content(self):
        ...
        self._p("AI-агента для написания кода. Каждый диалог с агентом — это «сессия», хранящаяся")
        self._p("в SQLite. Со временем база разрастается до сотен мегабайт из-за:")
        self._bullet("reasoning-токенов (Chain of Thought) — ~77% объёма")
        self._bullet("вызовов инструментов (read/write/edit/bash) с их параметрами и результатами")
        self._bullet("патчей (изменений файлов) и снимков версий (snapshot)")
        self._bullet("ошибок, дублей, недозавершённых сессий")
        self._p("")
        self._p("Программа решает эти проблемы, не требуя внешних зависимостей кроме Python 3.10+.")
        self._p("")

        self._h("1.1 Возможности", "h2")
        self._bullet("Просмотр сессий в иерархическом дереве (родитель → subagent)")
        self._bullet("Чат-формат сообщений с подсветкой типов контента")
        self._bullet("Архивация/разархивация сессий без потери данных")
        self._bullet("Удаление reasoning-токенов (сокращение БД на ~77%)")
        self._bullet("Экспорт/импорт сессий в JSON")
        self._bullet("Перенос сессий между проектами с каскадным обновлением subagent")
        self._bullet("Vacuum и очистка мусора (orphan-diff, снапшоты)")
        self._bullet("Выбор между несколькими БД (opencode.db / opencode-dev.db / копии)")
        self._bullet("MCP-инспектор для диагностики через opencode-сессию")
        self._p("")

        self._h("1.2 Технические характеристики", "h2")
        self._table_row(["Характеристика", "Значение"], header=True)
        self._table_row(["Язык", "Python 3.10+ (только stdlib)"])
        self._table_row(["GUI", "tkinter (ttk) — встроен в Python"])
        self._table_row(["БД", "SQLite 3 (прямое чтение, без посредников)"])
        self._table_row(["Платформы", "Windows, Linux, macOS"])
        self._table_row(["Зависимости", "Нет (кроме опционального mcp для MCP-сервера)"])
        self._table_row(["Размер", "~2000 строк кода (app.py + core.py + mcp-opencode-db.py)"])
        self._p("")

        self._h("1.3 Интерфейс", "h2")
        self._table_row(["Элемент", "Описание"], header=True)
        self._table_row(["Вкладка «Сессии»", "Список сессий в виде дерева, сортировка, поиск, экспорт/импорт"])
        self._table_row(["Вкладка «Сообщения»", "Чат-просмотр одной сессии, lazy-loading, удаление частей"])
        self._table_row(["Вкладка «Очистка»", "Массовые операции: strip reasoning, vacuum, удаление старых"])
        self._table_row(["Вкладка «Дашборд»", "Статистика хранилища, топ сессий, график разбивки"])
        self._table_row(["Вкладка «Справка»", "Полная документация (вы её читаете)"])
        self._table_row(["Строка меню", "Файл, Инструменты, Справка — быстрые команды"])
        self._table_row(["Статус-бар", "Текущее состояние: «[opencode] Загружено 60 сессий»"])
        self._p("")

        self._h("1.4 Структура файлов программы", "h2")
        self._code("  opencode-manager/")
        self._code("  ├── app.py               # GUI: все вкладки, кнопки, меню, тема")
        self._code("  ├── core.py              # Работа с БД: OpenCodeDB + OpenCodeCLI")
        self._code("  ├── mcp-opencode-db.py   # MCP-сервер для диагностики")
        self._code("  ├── run.py               # Точка входа")
        self._code("  ├── start.bat / .ps1     # Лаунчеры для Windows")
        self._code("  ├── CONTEXT_RULES.md     # Правила для AI-агента")
        self._code("  ├── docs/                # Вся документация с системой Doc-ID")
        self._code("  │   ├── ARCHITECTURE.md  # Архитектура OpenCode и менеджера")
        self._code("  │   ├── CHANGELOG.md     # История изменений")
        self._code("  │   ├── bugs/            # Трекинг багов")
        self._code("  │   └── features/        # Feature-документация")
        self._code("  └── test_core.py         # Тесты")
        self._sep()
        ...
        self._p("Главная вкладка. Показывает все сессии OpenCode из выбранной БД.")
        self._p("Каждая строка — одна сессия (диалог с AI-агентом).")
        self._p("Поддерживает иерархическое отображение: корневые сессии и их subagent-дети.")
        self._p("")

        self._h("2.1 Панель инструментов (верхняя)", "h2")
        self._p("Содержит элементы управления:")
        self._p("")
        self._b("Выбор БД (DB Selector):")
        self._p("Выпадающий список всех найденных opencode*.db в ~/.local/share/opencode/.")
        self._p("При запуске автоматически выбирается БД с наибольшим количеством сессий.")
        self._p("Переключение — сессии перезагружаются из выбранной БД в реальном времени.")
        self._code("  Пример: «opencode (60 сес.) | opencode-dev (7 сес.) | opencode1 (51 сес.)»")
        self._p("")
        self._b("Сортировка:")
        self._p("Выпадающий список: «По размеру», «По дате», «По имени».")
        self._p("Также можно кликнуть по любому заголовку столбца для сортировки.")
        self._p("")
        self._b("Поиск:")
        self._p("Фильтрует сессии по названию или ID в реальном времени (по мере ввода).")
        self._code("  Введите «qtest» → останутся только сессии с упоминанием qtest")
        self._code("  Введите «ses_183» → покажет сессии с ID, начинающимся на ses_183")
        self._p("")
        self._b("Чекбокс «Sub»:")
        self._p("Включает/выключает отображение subagent-сессий в дереве.")
        self._p("Выкл — показываются только корневые сессии (как в opencode).")
        self._p("Вкл — полное иерархическое дерево с детьми под родителями.")
        self._p("")
        self._b("Кнопка «Обновить»:")
        self._p("Принудительная перезагрузка списка сессий из БД (аналог F5).")
        self._p("")

        self._h("2.2 Колонки таблицы (подробно)", "h2")
        self._p("Таблица использует Treeview с column #0 для названия (с отступами иерархии).")
        self._p("")
        self._table_row(["Колонка", "Смысл", "Формат", "Пример"], header=True)
        self._table_row(["#0 (Название)", "Заголовок сессии", "Текст, до 60 символов",
                         "«DepSeek: новый подход»"])
        self._table_row(["Дочер.", "Число subagent-детей / → / ⚠",
                         "Число / → / ⚠", "«3» / «→» / «⚠»"])
        self._table_row(["Сост.", "Статус архивации", "🗄 или пусто",
                         "«🗄» = в архиве, «» = активна"])
        self._table_row(["Директория", "Путь к проекту", "Сокращённый путь",
                         "Q:\\...\\opencode-manager"])
        self._table_row(["Размер", "Сумма всех частей", "МБ / ГБ",
                         "142.1 MB / 2.3 GB"])
        self._table_row(["Сообщ.", "Кол-во сообщений", "Число",
                         "42"])
        self._table_row(["Tokens In", "Входящие токены", "Число с разделителем",
                         "1 234 567"])
        self._table_row(["Tokens Out", "Исходящие токены", "Число с разделителем",
                         "98 765"])
        self._table_row(["Reasoning", "Thinking-токены", "Число с разделителем",
                         "987 654"])
        self._table_row(["Возраст", "Относительное время", "today / Nd / Nw / Nmo",
                         "3d ago / 2w ago / 1mo ago"])
        self._table_row(["Модель", "ID модели AI", "provider/model",
                         "deepseek-v4-flash-free"])
        self._p("")
        self._tip("  Клик по заголовку → сортировка. Повторный клик → меняет направление ▲/▼.")
        self._p("")

        self._h("2.3 Типы строк в дереве", "h2")
        self._table_row(["Тип", "Цвет", "Стиль", "Колонка «Дочер.»", "Описание"], header=True)
        self._table_row(["Корневая", "Обычный", "Обычный",
                         "Число детей", "Родительская сессия, созданная пользователем"])
        self._table_row(["Subagent", "Серый (#8b949e)", "Курсив",
                         "→", "Дочерняя сессия (@explore / @general) под родителем"])
        self._table_row(["Orphan", "Красный (#dc2626)", "Обычный",
                         "⚠", "Subagent, чей родитель удалён"])
        self._table_row(["Архивная", "Серый (#8b949e)", "Обычный",
                         "Число/→", "Сессия скрыта из opencode (time_archived IS NOT NULL)"])
        self._p("")

        self._h("2.4 Выбор сессий", "h2")
        self._bullet("Одиночный клик — выбрать одну строку")
        self._bullet("Ctrl + Click — добавить/убрать из множественного выбора")
        self._bullet("Shift + Click — выбрать диапазон")
        ...
        self._h("8.2 Полная очистка перед переустановкой", "h2")
        self._code("  1. Закройте OpenCode")
        self._code("  2. Откройте Session Manager")
        ...
        self._tip("  Результат: минимальная БД, никакого мусора.")
        self._p("")
        ...
        self._h("8.4 Оставить только 5 последних", "h2")
        ...
        self._h("8.5 Перенести сессию в другой проект", "h2")
        ...
        self._h("8.6 Архивация сессий", "h2")
        ...
        self._h("8.7 Диагностика через MCP", "h2")
        ...
        self._h("8.8 Резервное копирование", "h2")
        self._b("Где лежит:")
        ...
        self._h("9. Частые вопросы (FAQ)", "h1")

        self._h("9.1 Что такое reasoning и почему он занимает 77% БД?", "h2")
        ...
        self._h("9.2 Что такое subagent-сессии?", "h2")
        ...
        self._h("9.3 Что такое session_diff?", "h2")
        ...
        self._h("9.4 Почему Vacuum удваивает размер БД?", "h2")
        ...
        self._h("9.5 Можно ли пользоваться менеджером при запущенном OpenCode?", "h2")
        ...
        self._h("9.6 Как переключиться между opencode.db и opencode-dev.db?", "h2")
        self._p("В тулбаре вкладки «Сессии» → выпадающий список «БД».")
        self._p("Выберите нужную — сессии перезагрузятся из неё.")
        self._p("")

        self._h("9.7 Почему я вижу 3 сессии вместо 60?", "h2")
        ...
        self._h("9.8 Как очистить осиротевшие diff'ы?", "h2")
        ...
        self._h("9.9 Как работает сортировка?", "h2")
        ...
        self._h("9.10 Что такое компактированные сообщения?", "h2")
        ...
        self._h("9.11 Как найти проблемный вызов инструмента?", "h2")
        ...
        self._h("9.12 Что такое orphan-сессии и почему они красные?", "h2")
        ...
        self._h("9.13 Как работает MCP-инспектор?", "h2")
        ...
        self._h("9.14 Как сделать бэкап всей БД?", "h2")
        self._p("Ручное копирование папки (см. Гайд 8.8).")
        self._p("Или экспорт отдельных сессий через «Экспорт всех».")
        self._p("Встроенной функции бэкапа всей БД пока нет.")
        self._p("")

        self._h("9.15 Как восстановить сессию из JSON?", "h2")
        ...
        self._sep()
        # ── 10. SQL Reference ──
        self._help_section_pos["sql_ref"] = self._help_text.index("end-1c")
        self._h("10. Справочник SQL-запросов", "h1")
        self._p("Типовые запросы для диагностики через MCP (oc_query) или прямой SQL.")
        self._p("")

        self._h("10.1 Базовые запросы", "h2")
        self._code("  -- Все сессии в БД")
        self._code("  SELECT COUNT(*) FROM session;")
        self._code("")
        self._code("  -- Корневые vs subagent")
        self._code("  SELECT 'roots', COUNT(*) FROM session WHERE parent_id IS NULL")
        self._code("  UNION SELECT 'children', COUNT(*) FROM session WHERE parent_id IS NOT NULL;")
        self._code("")
        self._code("  -- Сессии по проектам")
        self._code("  SELECT p.worktree, COUNT(s.id) FROM session s")
        self._code("  JOIN project p ON s.project_id = p.id GROUP BY p.id;")
        self._code("")
        self._code("  -- Самые тяжёлые сессии")
        self._code("  SELECT s.id, s.title, ROUND(SUM(LENGTH(p.data))/1048576.0, 1) AS mb")
        self._code("  FROM session s JOIN part p ON p.session_id = s.id")
        self._code("  GROUP BY s.id ORDER BY mb DESC LIMIT 10;")
        self._p("")

        self._h("10.2 Диагностика", "h2")
        self._code("  -- Orphan-дети (родитель удалён)")
        self._code("  SELECT id, parent_id FROM session s1")
        self._code("  WHERE parent_id IS NOT NULL")
        self._code("  AND NOT EXISTS (SELECT 1 FROM session s2 WHERE s2.id = s1.parent_id);")
        self._code("")
        self._code("  -- Архивные сессии")
        self._code("  SELECT id, title FROM session WHERE time_archived IS NOT NULL;")
        self._code("")
        self._code("  -- Сессии с некорректным directory")
        self._code("  SELECT id, directory FROM session WHERE directory LIKE '%/%';")
        self._p("")

        self._h("10.3 Статистика", "h2")
        self._code("  -- Размер reasoning от общего")
        self._code("  SELECT ROUND(SUM(CASE WHEN json_extract(data,'$.type')='reasoning'")
        self._code("    THEN LENGTH(data) ELSE 0 END) * 100.0 / NULLIF(SUM(LENGTH(data)),0), 1)")
        self._code("  AS reasoning_pct FROM part;")
        self._code("")
        self._code("  -- Части по типам")
        self._code("  SELECT json_extract(data, '$.type') AS type,")
        self._code("    COUNT(*) AS count, ROUND(SUM(LENGTH(data))/1048576.0, 1) AS mb")
        self._code("  FROM part GROUP BY type ORDER BY mb DESC;")
        self._p("")

        self._sep()
        # ── 11. Troubleshooting ──
        self._help_section_pos["trouble"] = self._help_text.index("end-1c")
        self._h("11. Диагностика проблем", "h1")

        self._h("11.1 Сессия не отображается в opencode", "h2")
        self._p("1. Проверьте какая БД активна: `opencode db path`")
        self._p("2. Проверьте не архивирована ли сессия: `WHERE time_archived IS NOT NULL`")
        self._p("3. Проверьте project_id: если 'global' — может не совпадать проект")
        self._p("4. Проверьте directory: если есть / вместо \\ — исправьте через перенос")
        self._p("")

        self._h("11.2 Desktop не запускается", "h2")
        self._p("1. Проверьте renderer.log: %APPDATA%\\ai.opencode.desktop\\logs\\")
        self._p("2. Если ошибка 'store-set' — BOM в global.dat (см. 2.14.3)")
        self._p("3. Если ошибка SQLITE_BUSY — удалите .db-wal и .db-shm")
        self._p("")

        self._h("11.3 БД заблокирована", "h2")
        self._p("Закройте все процессы OpenCode:")
        self._code("  Get-Process -Name 'OpenCode','opencode' | Stop-Process")
        self._p("Если не помогает — удалите WAL-файлы:")
        self._code("  del %USERPROFILE%\\.local\\share\\opencode\\opencode.db-wal")
        self._code("  del %USERPROFILE%\\.local\\share\\opencode\\opencode.db-shm")
        self._p("")

        self._h("11.4 MCP не загружается", "h2")
        self._p("1. Проверьте полный путь к python.exe в opencode.jsonc")
        self._p("2. Установите mcp: `pip install mcp`")
        self._p("3. Проверьте JSONC-синтаксис конфига")
        self._p("4. Перезапустите opencode")
        self._p("")

        self._h("11.5 Кнопки архивирования/удаления неактивны", "h2")
        self._p("Закройте OpenCode — менеджер блокирует операции записи при запущенном OpenCode.")
        self._p("")

        self._sep()
        # ── 12. Glossary ──
        self._help_section_pos["glossary"] = self._help_text.index("end-1c")
        self._h("12. Глоссарий", "h1")
