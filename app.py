...
class App(tk.Tk):
    ...
    def _build_help_content(self):
        ...
        self._table_row(["Директория", "Путь к проекту сессии (сокращённый: Q:\\...\\ → имя папки)"])
        ...
        self._h("2.8 Перенос сессии в другой проект", "h1")

        self._h("2.8.1 Как работает", "h2")
        self._p("Кнопка «Перенести в проект» меняет поля сессии в БД:")
        self._bullet("directory — новый путь (backslashes на Windows)")
        self._bullet("path — тот же путь с forward slashes")
        self._bullet("project_id — ID целевого проекта (или 'global')")
        self._p("Дочерние subagent-сессии (с parent_id) переносятся вместе с родителем.")
        self._p("")

        self._h("2.8.2 Защита от запущенного OpenCode", "h2")
        self._p("Перед переносом менеджер проверяет, запущен ли OpenCode:")
        self._code("  Проверка через: Get-Process -Name 'OpenCode.exe','opencode'")
        self._p("Если процесс обнаружен — операция блокируется, выводится предупреждение и звуковой сигнал:")
        self._warn("  OpenCode запущен. Закройте его перед переносом!")
        self._p("Это предотвращает race condition и повреждение БД.")
        self._p("")

        self._h("2.8.3 Известные проблемы", "h2")
        self._p("1. Tkinter на Windows может вернуть путь с forward slashes из диалога выбора папки.")
        self._p("   Исправлено: нормализация в app.py и core.py (replace('/', '\\\\')).", tag="indent")
        self._p("2. После переноса сессия меняет project_id и исчезает из старого проекта.", tag="indent")
        self._p("   Чтобы увидеть её — откройте проект через Desktop (Ctrl+O).", tag="indent")
        self._p("3. Если сессия не отображается — проверьте формат directory (должен быть с \\).", tag="indent")
        self._p("")

        self._h("2.9 Столбец «Директория»", "h1")
        self._p("Показывает путь к проекту сессии. Автоматически сокращается:")
        self._code("  Q:\\User_Data\\Desktop\\opencode-manager → opencode-manager")
        self._code("  C:\\Users\\misch\\projects\\my-app → ~\\projects\\my-app")
        ...
        self._p("Менеджер автоопределяет БД по количеству сессий (приоритет opencode.db).")
